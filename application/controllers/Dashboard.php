...
class Dashboard extends CI_Controller
{
  ...
  public function index()
  {
    ...
    foreach ($history as $key => $value) {
      $checkUser = $this->Db_select->query('select *from user a join company b on a.id_company = b.id where a.id = '.$value->from);
      if ($checkUser->type_gameplay == $administrator['role']) {
